 ...
 *成绩注册完成Action
 */
public class ScoreRecordFinishAction extends ActionSupport {
	...
	public String execute() throws Exception {
		//以下为测试数据
		/*System.out.println(courseID+"----->"+examTime+"------->"+examType+"------->"+xueqi);
		for(int i=0; i<stuXueHao.size(); i++)
		{
			System.out.println(stuXueHao.get(i)+"-------->"+score.get(i));
		}*/
		//以上为测试数据
		
		//以下为测试数据
		/*for (int i = 0; i < scores.size(); i++) {
			System.out.println(scores.get(i).getStuXueHao()+ "------>" + scores.get(i).getScore());
		}*/
		//以上为测试数据
		...
		//以下为测试数据
			...
			System.out.println("总人数" + stuCount);
			System.out.println("平均分" + avgScore);
			System.out.println("最高分" + maxScore);
			System.out.println("最低分" + minScore);
			System.out.println("不及格人数" + failCount);
			System.out.println("及格人数" + sucCount);
			System.out.println("良好人数" + goodCount);
			System.out.println("中等人数" + secondaryCount);
